...
Base docker image to run a MySQL database server. Based on tutum docker mysql image.


Usage
-----

To create the image `dordoka/rpi-mysql`, execute the following command on the docker-rpi-mysql folder:

        docker build -t dordoka/rpi-mysql 

To run the image and bind to port 3306:

        docker run -d -p 3306:3306 dordoka/rpi-mysql
...
set the environment variable `MYSQL_PASS` to your specific password when running the container:
        ...
        docker run -d -p 3306:3306 -e MYSQL_PASS="mypass" dordoka/rpi-mysql
...
on the container to store the database files. To do so:
        ...
        docker run -d -v /path/in/host:/var/lib/mysql dordoka/rpi-mysql /bin/bash -c "/usr/bin/mysql_install_db"
...
To do so, first create a container that holds database files:
    ...
    docker run -d -v /var/lib/mysql --name db_vol -p 22:22 resin/rpi-raspbian:jessie 
...
After this you can start your MySQL image using volumes in the container created above (put the name of container in `--volumes-from`)
    ...
    docker run -d --volumes-from db_vol -p 3306:3306 dordoka/rpi-mysql 
...
To import a SQL backup which is stored for example in the folder `/tmp` in the host, run the following:
        ...
        sudo docker run -d -v /tmp:/tmp dordoka/rpi-mysql /bin/bash -c "/import_sql.sh <user> <pass> /tmp/<dump.sql>"

Also, you can start the new database initializing it with the SQL file:

        sudo docker run -d -v /path/in/host:/var/lib/mysql -e STARTUP_SQL="/tmp/<dump.sql>" dordoka/rpi-mysql
...
- Master MySQL
- 
        docker run -d -e REPLICATION_MASTER=true -e REPLICATION_PASS=mypass -p 3306:3306 --name mysql dordoka/rpi-mysql

- Example on Slave MySQL:
- 
        docker run -d -e REPLICATION_SLAVE=true -p 3307:3306 --link mysql:mysql dordoka/rpi-mysql
...
Kudos
-----
To Tutum, for the docker mysql image.
To Hypriot, for the awesome raspbian based distro that includes docker.
